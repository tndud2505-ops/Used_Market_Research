...
export class JobRunner {
  ...
  async executeJob(job: JobPlan, jobId: string): Promise<JobExecutionResult> {
    trace("scheduler.job:start", { job_name: job.name, job_id: jobId });
    const startTime = new Date().toISOString();
    const attempts: JobAttempt[] = [];
    const retryPolicy = resolveRetryPolicy(job);
    ...
    for (let attempt = 1; attempt <= retryPolicy.max_attempts; attempt++) {
      ...
      try {
        lastPayload = await this.runJobLogic(job, jobId);
        ...
        attempts.push({
          ...
          delay_ms: attempt < retryPolicy.max_attempts ? calculateBackoffDelay(attempt + 1, retryPolicy) : undefined
        });
        ...
        if (!isRetryableError(errorType, retryPolicy) || attempt === retryPolicy.max_attempts) {
          break;
        }
      }
    }
  ...
  async executeBatch(jobsToRun?: JobPlan[]): Promise<BatchExecutionResult> {
    ...
    for (const job of jobPlans) {
      ...
      try {
        jobResults.push(await this.executeJob(job, jobId));
      } catch (error: unknown) {
        const errorMsg = error instanceof Error ? error.message : String(error);
    ...
    await writeCentralResult({
      ...
      summary: {
        status: metadata.failed_jobs > 0 ? "partial_success" : "success",
  ...
  private async runJobLogic(job: JobPlan, jobId: string): Promise<unknown> {
  ...
  private extractErrorType(error: unknown): string {
    ...
    if (errorMsg.includes("VALIDATION") || errorMsg.includes("validation")) return "VALIDATION_ERROR";
    if (errorMsg.includes("auth")) return "AUTHENTICATION_ERROR";
