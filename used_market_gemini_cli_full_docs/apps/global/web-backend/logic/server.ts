...
import { runWebSearch, webSearchExecutionKey, WebSearchValidationError } from './search-service.js';
...
function publicCategoryCatalog() {
  return categoryCatalogForApi();
}

type JsonRecord = Record<string, unknown>;

function runnerBearerTokenMatches(authorization: string | undefined, expectedToken: string) {
  const prefix = 'Bearer ';
...
const PUBLIC_SITE_ORIGIN = 'https://global.used-pick.com';
...
export function createServer(
  ...
  const resolvedOptions = {
    host: options.host?.trim() || WEB_BACKEND_CONFIG.host,
  ...
  const runnerOriginToken = process.env.CLOUDFLARE_RUNNER_TOKEN?.trim() ?? '';
  ...
  const server = http.createServer(async (req, res) => {
    ...
    res.setHeader('X-Used-Market-App', 'global');
    ...
    try {
      ...
      if (urlObj.pathname !== '/global' && !urlObj.pathname.startsWith('/global/')) {
        throw new ApiError(404, 'Not found');
      }
      if (runnerOriginToken && urlObj.pathname !== '/global/health'
        && !runnerBearerTokenMatches(req.headers.authorization, runnerOriginToken)) {
        throw new ApiError(401, 'Unauthorized', 'Cloudflare runner token is missing or invalid');
      }
      if (urlObj.pathname === '/global') {
        res.statusCode = 308;
        res.setHeader('Location', '/global/');
        return res.end();
      }
      const pathname = urlObj.pathname.slice('/global'.length) || '/';
      ...
      if (pathname === '/api/search') {
        ...
        const releaseSlot = acquireSearchSlot(`web:${webSearchExecutionKey(payload)}`, true);
  ...
  server.listen(port, resolvedOptions.host, () => {
    console.log(`[web-backend] Listening on ${resolvedOptions.host}:${port}`);
...
async function serveStaticAsset(pathname: string, requestUrl: URL, res: http.ServerResponse, headOnly = false) {
  ...
  if (pathname === '/') {
    const country = requestUrl.searchParams.get('country') === 'us' ? 'us' : 'jp';
    content = Buffer.from(renderGlobalIndexHtml(content.toString('utf8'), country), 'utf8');
  }
  res.statusCode = 200;
...
function renderGlobalIndexHtml(html: string, country: 'jp' | 'us') {
  const canonicalUrl = `${PUBLIC_SITE_ORIGIN}/global/?country=${country}`;
