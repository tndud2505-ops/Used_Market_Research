...
import {
  ...
  SearchSessionGenerationMismatchError,
  SearchSessionStore,
  SearchSessionWindowInvalidError,
...
const FOREIGN_BROWSER_SITES = ['mercari_jp', 'yahoo_auction_jp', 'rakuma', 'poshmark', 'vinted', 'unclaimed_baggage', 'ebay'];
const KEYWORD_ONLY_SITES = new Set(FOREIGN_BROWSER_SITES);
const DEFAULT_SITES = FOREIGN_BROWSER_SITES;
const SUPPORTED_WEB_SEARCH_SITES = new Set(FOREIGN_BROWSER_SITES);
...
export function createWebSearchRunner(options: WebSearchRunnerOptions = {}) {
  ...
  const sessionStore = options.sessionStore ?? new SearchSessionStore({ now });
  ...
  return async (input: Record<string, unknown>): Promise<Record<string, unknown>> => {
    ...
    const sessionIdentity = searchSessionIdentity(request);

    if (request.sessionId) {
      const current = sessionStore.read(request.sessionId);
      if (!current) {
        throw new WebSearchValidationError('SESSION_EXPIRED: Search session expired. Start a new search.');
      }
      assertSearchSessionMatches(current, request, sessionIdentity);
      assertSearchSessionWindow(current, request.sessionWindow);

      if (request.sessionOnly || input.cursor === undefined) {
        const viewed = request.sessionWindow !== null && request.sessionWindow > current.windowLimit
          ? advanceSearchSessionWindow(sessionStore, current.id, request.sessionWindow)
          : current;
        return buildSearchSessionPayload(viewed, request);
      }

      if (input.cursor !== current.pagination.next_cursor) {
        throw new WebSearchValidationError('SESSION_MISMATCH: Continuation cursor does not match the current search session.');
      }
      ...
      const continuation = await collect(collectionRequest);
      if (continuation.status !== 'success') return continuation;
      let appended: SearchSessionSnapshot | null;
      try {
        appended = sessionStore.append(
          request.sessionId,
          asRecord(continuation.data),
          request.sessionGeneration ?? undefined
        );
      } catch (error) {
        if (error instanceof SearchSessionGenerationMismatchError) {
          throw new WebSearchValidationError('SESSION_MISMATCH: Search session generation is stale.');
        }
        throw error;
      }
      if (!appended) {
      ...
      if (request.sessionWindow !== null && request.sessionWindow > appended.windowLimit) {
        appended = advanceSearchSessionWindow(sessionStore, appended.id, request.sessionWindow);
      }
      return buildSearchSessionPayload(appended, request);
    }
    ...
    if (basePayload.status !== 'success') return basePayload;
    const created = sessionStore.create(sessionIdentity, asRecord(basePayload.data));
    return buildSearchSessionPayload(created, request);
  };
}
...
export function webSearchExecutionKey(input: Record<string, unknown>) {
  const request = validateWebSearchRequest(input);
  return request.sessionId
    ? `session:${request.sessionId}`
    : `collection:${collectionKeyForValidatedRequest(request)}`;
}
...
function searchSessionIdentity(request: ValidatedWebSearchRequest) {
  return JSON.stringify({
    keyword: request.keyword,
    effective_keyword: request.effectiveKeyword,
    category_ids: [...request.categoryIds].sort(),
    sites: [...request.sites].sort(),
    limit: request.limit
  });
}

function assertSearchSessionMatches(
  session: SearchSessionSnapshot,
  request: ValidatedWebSearchRequest,
  identity: string
) {
  if (session.identity !== identity) {
    throw new WebSearchValidationError('SESSION_MISMATCH: Search session does not match this query, category, or source set.');
  }
  if (request.sessionGeneration !== null && request.sessionGeneration !== session.generation) {
    throw new WebSearchValidationError('SESSION_MISMATCH: Search session generation is stale.');
  }
}

function assertSearchSessionWindow(session: SearchSessionSnapshot, requestedWindow: number | null) {
  if (requestedWindow !== null && requestedWindow > session.windowLimit + 160) {
    throw new WebSearchValidationError('SESSION_WINDOW_INVALID: Search session window can increase by at most 160 items.');
  }
}

function advanceSearchSessionWindow(store: SearchSessionStore, id: string, requestedWindow: number) {
  try {
    const advanced = store.advanceWindow(id, requestedWindow);
    if (!advanced) {
      throw new WebSearchValidationError('SESSION_EXPIRED: Search session expired. Start a new search.');
    }
    return advanced;
  } catch (error) {
    if (error instanceof SearchSessionWindowInvalidError) {
      throw new WebSearchValidationError('SESSION_WINDOW_INVALID: Search session window can increase by at most 160 items.');
    }
    throw error;
  }
...
function buildSearchSessionPayload(session: SearchSessionSnapshot, request: ValidatedWebSearchRequest) {
  const sessionWindow = Math.min(
    session.windowLimit,
    session.items.length,
    SEARCH_SESSION_MAX_ITEMS
  );
  const windowItems = session.items.slice(0, sessionWindow);
  const viewSites = request.viewSites ?? request.sites;
  const viewedItems = windowItems.filter((item) => viewSites.includes(readString(item.site, 'unknown')));
  const controlled = applySearchControls(viewedItems, {
    sort: request.sort,
    minPrice: request.minPrice,
    maxPrice: request.maxPrice
  });
  const availableItems = controlled.items;
  const pageCount = Math.max(1, Math.ceil(availableItems.length / SEARCH_SESSION_PAGE_SIZE));
  const page = Math.min(request.sessionPage, pageCount - 1);
  const pageStart = page * SEARCH_SESSION_PAGE_SIZE;
  const items = availableItems.slice(pageStart, pageStart + SEARCH_SESSION_PAGE_SIZE);
  const sourceTotals = Object.fromEntries(request.sites.map((site) => [
    site,
    availableItems.filter((item) => readString(item.site, 'unknown') === site).length
  ]));
  const sources = session.sources.map((source) => {
    const key = readString(source.key, 'unknown');
    const total = sourceTotals[key] ?? 0;
    return {
      ...source,
      count: total,
  ...
  const priceSummary = buildTrustedPriceSummary(availableItems);
  const baseSummary = asRecord(session.data.summary);
  const baseQuality = asRecord(session.data.quality);
  ...
  return {
    ...
    data: {
      ...
      summary: {
        ...
        source_count: Object.values(sourceTotals).filter((total) => total > 0).length,
        ...priceSummary,
        suspect_count: availableItems.filter((item) => item.price_suspect === true
          || item.noise_filtered === true
          || (typeof item.fraud_risk === 'number' && item.fraud_risk > 0.45)).length
      },
      quality: {
        ...baseQuality,
        merged_count: windowItems.length,
        available_count: availableItems.length,
        filtered_out_count: windowItems.length - availableItems.length
      },
      session: {
        id: session.id,
        generation: session.generation,
        page,
        page_size: SEARCH_SESSION_PAGE_SIZE,
        window: sessionWindow,
        loaded_count: session.items.length,
        available_count: availableItems.length,
        source_totals: sourceTotals,
...
async function runSingleWebSearch(
  ...
  const workflow = await orchestrator.fullWorkflow({
    ...
    persistMarketResult: !request.sites.includes('ebay')
  });
  ...
  const resultBySite = new Map(searchResults.map((value) => {
    const result = asRecord(value);
    return [readString(result.site, "unknown"), result] as const;
  }));
  const nextSiteCursors = Object.fromEntries(request.sites.map((site) => {
    const result = resultBySite.get(site);
    if (!result) return [site, EXHAUSTED_SITE_CURSOR];
    const nextCursor = asRecord(result.pagination).next_cursor;
    return [site, typeof nextCursor === "string" && nextCursor ? nextCursor : EXHAUSTED_SITE_CURSOR];
  }));
  ...
  const sources = sourceSummaries.map((source) => {
    ...
    if (!hasErrors && source.count > 0 && visibleCount === 0 && !controlled.filter_meta.applied) {
      warnings.push('Collection completed, but no active listings are available to display.');
    }
...
export function validateWebSearchRequest(input: Record<string, unknown>) {
  ...
  if (uniqueSites.length === 0 || uniqueSites.length > 7) {
    throw new WebSearchValidationError('sites must contain between 1 and 7 supported sites');
  }
  ...
  const sessionId = input.session_id === undefined
    ? null
    : typeof input.session_id === 'string' && /^[0-9a-f]{8}-[0-9a-f-]{27}$/i.test(input.session_id.trim())
      ? input.session_id.trim()
      : (() => { throw new WebSearchValidationError('session_id is invalid'); })();
  const sessionGeneration = input.session_generation === undefined
    ? null
    : typeof input.session_generation === 'number' && Number.isInteger(input.session_generation) && input.session_generation >= 1
      ? input.session_generation
      : (() => { throw new WebSearchValidationError('session_generation must be a positive integer'); })();
  if (sessionId && sessionGeneration === null) {
    throw new WebSearchValidationError('session_id requires session_generation');
  }
  if (!sessionId && sessionGeneration !== null) {
    throw new WebSearchValidationError('session_generation requires session_id');
  }
  if (input.cursor !== undefined && !sessionId) {
    throw new WebSearchValidationError('cursor requires session_id');
  }
  const sessionPage = input.session_page === undefined
    ? 0
    : typeof input.session_page === 'number' && Number.isInteger(input.session_page) && input.session_page >= 0
      ? input.session_page
      : (() => { throw new WebSearchValidationError('session_page must be a non-negative integer'); })();
  if (input.session_only !== undefined && typeof input.session_only !== 'boolean') {
    throw new WebSearchValidationError('session_only must be a boolean');
  }
  const sessionOnly = input.session_only === true;
  if (sessionOnly && !sessionId) {
    throw new WebSearchValidationError('session_only requires session_id');
  }
  if (sessionOnly && input.cursor !== undefined) {
    throw new WebSearchValidationError('session_only cannot include cursor');
  }
  const sessionWindow = input.session_window === undefined
    ? null
    : typeof input.session_window === 'number'
      && Number.isInteger(input.session_window)
      && input.session_window >= 1
      && input.session_window <= SEARCH_SESSION_MAX_ITEMS
      ? input.session_window
      : (() => { throw new WebSearchValidationError('session_window must be between 1 and 1000'); })();
  if (input.view_sites !== undefined && !Array.isArray(input.view_sites)) {
    throw new WebSearchValidationError('view_sites must be an array');
  }
  const viewSites = input.view_sites === undefined
    ? null
    : Array.from(new Set(input.view_sites
      .filter((site): site is string => typeof site === 'string')
      .map((site) => site.trim())
      .filter(Boolean)));
  if (viewSites && (viewSites.length === 0 || viewSites.some((site) => !uniqueSites.includes(site)))) {
    throw new WebSearchValidationError('view_sites must be a non-empty subset of sites');
  }
...
function toWebItem(value: unknown) {
  ...
  return {
    ...
    title: readString(item.title, 'Untitled listing'),
    price,
...
function validateSiteCursor(site: string, cursor: string) {
  if (cursor === EXHAUSTED_SITE_CURSOR) return;
  const pagination = resolveBrowserSiteAdapter(site).searchPagination ?? 'none';
  const offsetMatch = pagination === 'offset' ? cursor.match(/^offset:(\d+)$/) : null;
  const validOffset = offsetMatch !== null && Number(offsetMatch[1]) <= 10_000;
  const pageMatch = cursor.match(/^page:(\d+)(?::offset:(\d+))?$/);
  const page = pageMatch ? Number(pageMatch[1]) : null;
  const withinPageOffset = pageMatch?.[2] === undefined ? null : Number(pageMatch[2]);
  const validPage = page !== null
    && page >= 1
    && page <= 50
    && (withinPageOffset === null || withinPageOffset <= 10_000)
    && (pagination === 'page' || (pagination === 'none' && page === 1 && withinPageOffset !== null));
  const validAggregate = /^aggregate:v1:[A-Za-z0-9_-]{1,12000}$/.test(cursor);
  const valid = validOffset || validPage || validAggregate;
  if (!valid) throw new WebSearchValidationError(`cursor is invalid or expired for ${site}`);
}

const EXHAUSTED_SITE_CURSOR = 'exhausted:v1';
...
export function buildTrustedPriceSummary(items: Array<{
  ...
  const currency = currencies.size === 1 ? Array.from(currencies)[0] : currencies.size > 1 ? 'MIXED' : 'USD';
  if (currency === 'MIXED') {
