...
if [[ -f "$compose_file" ]]; then
  docker compose -f "$compose_file" exec -T used-market-global sh -eu -c \
...
mapfile -t stale_images < <(docker image ls --quiet --filter dangling=true --filter label=com.docker.compose.project=used-market-global-app | sort -u)
if (( ${#stale_images[@]} > 0 )); then
  docker image rm "${stale_images[@]}" || true
fi

if (( result_status == 0 )); then
  echo "global retention: ok (results>${retention_days}d)"
else
  echo "global retention failed with ${result_status}" >&2
fi
exit "$result_status"
