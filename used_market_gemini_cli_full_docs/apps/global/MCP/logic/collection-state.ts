...
export function deriveCollectionState(input: CollectionStateInput): CollectionState {
  ...
  const hasDegradedWarning = warnings.some((value) =>
    /HIGH_FILTER_RATE|RATE[_ -]?LIMIT|TIMEOUT|SELECTOR_DRIFT|BLOCKED_PAGE/i.test(value)
  );

  if (errors.length > 0) return input.itemCount > 0 ? "partial" : "failed";
  if (input.itemCount > 0) return hasDegradedWarning ? "partial" : "ready";
