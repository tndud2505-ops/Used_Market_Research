...
import { validateCategory, validateKeyword, validateLimit, validateSearchInput, validateSite, validateSites } from "./validation.js";
import {
...
export class Orchestrator {
  ...
  async loginCheck(siteKey: string, options?: BrowserCommandOptions) {
    ...
    const result = LoginCheckResultSchema.parse(await collectLoginCheck(siteKey, { showBrowser, signal: options?.signal }));
  ...
  async search(input: SearchCommandInput, options?: BrowserCommandOptions) {
    validateSearchInput(input);
    const showBrowser = this.resolveShowBrowser(options);
    trace("orchestrator.search:start", { ...input, show_browser: showBrowser });
    options?.signal?.throwIfAborted();
    const collected = SearchResultSchema.parse(await collectSearchListings(input, {
      showBrowser,
      signal: options?.signal
    }));
    const result = normalizeRawResult(SearchResultSchema.parse({
      ...collected,
      keyword_is_explicit: input.keywordIsExplicit !== false
    }));
    trace("orchestrator.search:complete", {
      site: input.site,
      keyword: input.keyword,
      category: input.category?.id ?? null,
  ...
  async normalize(siteKey: string, _keyword: string, searchResult: SearchResult, options?: BrowserCommandOptions) {
    validateSite(siteKey);
    resolveSite(siteKey);
    options?.signal?.throwIfAborted();
    ...
    const detailEnrichment = await enrichBuildListingDetails(siteKey, searchResult, { signal: options?.signal });
  ...
  async fullWorkflow(input: FullWorkflowInput, options?: BrowserCommandOptions) {
    ...
    trace("orchestrator.fullWorkflow:start", {
      ...
      category: input.category?.id ?? null,
      site_cursors: input.siteCursors ?? {},
      show_browser: showBrowser
    });

    for (const siteKey of input.sites) {
      options?.signal?.throwIfAborted();
      if (input.siteCursors?.[siteKey] === "exhausted:v1") {
        trace("orchestrator.fullWorkflow:site:skip", { site: siteKey, reason: "exhausted" });
        continue;
      }
      ...
      let login = LoginCheckResultSchema.parse({
        site: siteKey,
        login_status: "unknown",
        current_page: "",
        notes: "Login check skipped for public search.",
        errors: []
      });
      if (site.loginRequired) {
        login = LoginCheckResultSchema.parse(await this.loginCheck(siteKey, { showBrowser, signal: options?.signal }));
      }
      ...
      const search = await this.search({
        site: siteKey,
        keyword: input.keyword,
        keywordIsExplicit: input.keywordIsExplicit,
        limit: input.limit,
        category: input.category,
        cursor: input.siteCursors?.[siteKey] ?? null
      }, { showBrowser, signal: options?.signal });
      searchResults.push(search);
      normalizedResults.push(await this.normalize(siteKey, input.keyword, search, { signal: options?.signal }));
