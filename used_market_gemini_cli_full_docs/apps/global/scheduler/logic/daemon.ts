...
import { getSchedulerPriorityRank, JobRunner, type SchedulerNotifierEvent } from "./job-runner.js";
import type { JobPlan } from "./jobs.js";
import { validateJob } from "./schedule-config.js";
...
import { acquireSchedulerJobLock } from "./job-lock.js";
...
export class SchedulerDaemon {
  ...
  start(): RegisteredJobSchedule[] {
    ...
    this.registeredSchedules = this.jobPlans.map((job) => {
      ...
      const task = cron.schedule(
        cronExpression,
        () => {
          void this.executeScheduledJob(job).catch((error) => {
            traceError(`scheduler.daemon:scheduled-callback:failed:${job.name}`, error);
          });
        },
        {
          timezone: this.timezone
    ...
    this.persistStatus();
    void this.runAllNow().catch((error) => {
      traceError("scheduler.daemon:initial-run:failed", error);
    });
    return [...this.registeredSchedules];
  }
  ...
  async runAllNow(): Promise<void> {
    ...
    const orderedJobs = await Promise.all(this.jobPlans.map(async (job) => {
      try {
        return { job, priority: (await validateJob(job)).priority };
      } catch {
        return { job, priority: "medium" as const };
      }
    }));
    orderedJobs.sort((left, right) => getSchedulerPriorityRank(left.priority) - getSchedulerPriorityRank(right.priority));
  ...
  private async executeScheduledJob(job: JobPlan): Promise<void> {
    ...
    let jobLock;
    try {
      jobLock = await acquireSchedulerJobLock(job.name);
    } catch (error) {
      state.run_count += 1;
      ...
      this.persistStatus();
      traceError(`scheduler.daemon:lock:failed:${job.name}`, error);
      this.options.onJobError?.(job, error);
      return;
    }
    if (!jobLock) {
      trace("scheduler.daemon:job:skip-already-running", { job_name: job.name, scope: "process" });
      return;
    }
    ...
    try {
      ...
      if (alertDispatch.failed > 0) {
        postprocessWarnings.push(`alert_dispatch_failed:${alertDispatch.failed}`);
      }
      if (shouldTriggerReporterAfterJob(job) && (result.status === "success" || result.status === "partial_success")) {
        ...
        try {
          if (this.options.runReporterAfterJob) {
            ...
            const reporterResult = await createReporterDaemon().runOnce(reporterOptions);
            if (reporterResult.status !== "success") {
              postprocessWarnings.push(`reporter_${reporterResult.status}`);
            } else {
              reporterTriggered = true;
            }
          }
        } catch (reporterError) {
          postprocessWarnings.push(`reporter_failed:${reporterError instanceof Error ? reporterError.message : String(reporterError)}`);
          traceError(`scheduler.daemon:reporter:failed:${job.name}`, reporterError);
        }
      }
      const finalStatus = result.status === "failed" || result.status === "validation_failed"
        ? result.status
        : postprocessWarnings.length > 0
          ? "partial_success"
          : result.status;
      state.last_status = finalStatus;
      state.last_error = postprocessWarnings.join("; ") || result.error;
      const postprocess = {
      ...
      if (result.result_file) {
        await updateCentralRunSummary(result.result_file, {
          status: finalStatus === "validation_failed" ? "failed" : finalStatus,
          postprocess
        });
      }
      this.persistStatus();
      ...
      this.options.onJobComplete?.(job, runId, {
        ...result.notifier_event,
        status: finalStatus === "failed" || finalStatus === "validation_failed" ? "failed" : finalStatus === "partial_success" ? "warning" : "success"
      });
    } catch (error) {
      state.run_count += 1;
      state.last_finished_at = new Date().toISOString();
