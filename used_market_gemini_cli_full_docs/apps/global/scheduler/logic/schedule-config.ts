...
export function createSchedulerMetadata(
  ...
  const successfulCount = jobResults.filter((j) => j.status === "success").length;
  const partialCount = jobResults.filter((j) => j.status === "partial_success").length;
  ...
  return {
    ...
    status: failedCount > 0 ? "failed" : partialCount > 0 ? "partial_success" : "success",
    ...
    partial_jobs: partialCount,
    success_ratio: jobResults.length > 0 ? (successfulCount + partialCount) / jobResults.length : 0,
