...
>;
type CategoryAwareNoiseCandidate = NoiseFilterCandidate & { canonical_category_id?: string };

type AnnotatedNoiseCandidate = CategoryAwareNoiseCandidate & {
...
function resolveStaleListingThresholdMs(_item: NoiseFilterCandidate) {
  return null;
}
...
export function classifyNoiseCandidate(item: CategoryAwareNoiseCandidate): NoiseFilterReason | null {
  ...
  // The original noise rules protect a PC-parts market. Category-scoped
  // collection is broader, so do not prune clothing, appliances, etc. as
  // "non_pc_product" just because they have no PC components.
  if (item.canonical_category_id && item.canonical_category_id !== "pc") {
    return null;
  }
