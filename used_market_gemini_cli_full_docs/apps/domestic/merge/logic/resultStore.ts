...
export async function updateCentralRunSummary(
  baseDir: string,
  patch: Record<string, unknown>
) {
  const summaryPath = path.join(baseDir, "run-summary.json");
  const existing = JSON.parse(await readFile(summaryPath, "utf-8")) as Record<string, unknown>;
  const updated = { ...existing, ...patch, updated_at: new Date().toISOString() };
  await writeFile(summaryPath, JSON.stringify(updated, null, 2), "utf-8");
  return updated;
}
