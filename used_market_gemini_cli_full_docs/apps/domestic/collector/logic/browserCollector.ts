...
export function hasExplicitEmptySearchEvidence(adapter: BrowserSiteAdapter, pageHtml: string): boolean {
  if (adapter.siteKey !== "daangn" || pageHtml.trim() === "" || hasBlockedPageSignals(pageHtml)) {
    return false;
  }

  return /fleamarketArticles\s*["']?\s*:\s*\[\s*\]/i.test(pageHtml)
    || /(?:searchResults|search_results|items)\s*["']?\s*:\s*\[\s*\]/i.test(pageHtml)
    || /검색\s*(?:결과|상품)\s*가\s*없습니다|등록된\s*(?:상품|매물)\s*이\s*없습니다/i.test(pageHtml);
}
...
async function extractSearchItems(
  ...
  if (
    ...
    return SearchResultSchema.parse({
      ...
      items,
      warnings: warnings.length > 0 ? [`${sourceResults.length}개 사이트 카테고리 ID를 합산했습니다`, ...warnings] : [],
      quality_meta: {
  ...
  const pageHtml = await session.html();
  const publicSearchInput = categoryPlan?.strategy === "keyword" && adapter.siteKey === "bunjang"
    ? { ...input, category: undefined, keyword: categoryKeyword }
    : categoryPlan?.strategy === "source_category" && adapter.siteKey === "bunjang"
      ? { ...input, sourceCategoryId }
      : input;
  const publicSearchResult = await tryExtractPublicSearchResult(adapter, publicSearchInput, pageHtml);
