...
function canonicalizeListingUrl(value: string) {
  try {
    ...
    // Preserve an invalid URL so the required-field validation can report it.
  }
  return value;
}
...
export function normalizeRawResult(input: SearchResult): SearchResult {
  ...
  const newlyFilteredCount = Math.max(0, filteredCount - input.quality_meta.filtered_count);
  if (input.items.length > 0 && newlyFilteredCount / input.items.length >= 0.5) {
    warnings.push(`HIGH_FILTER_RATE:${newlyFilteredCount}/${input.items.length}`);
  }
  ...
  return SearchResultSchema.parse({
    ...
    collection_state: deriveCollectionState({
      ...
      extractedCount: input.items.length,
      filteredCount,
      warnings,
      errors: input.errors
    }),
    ...
    warnings,
    quality_meta: {
      extracted_count: Math.max(input.quality_meta.extracted_count, input.items.length),
