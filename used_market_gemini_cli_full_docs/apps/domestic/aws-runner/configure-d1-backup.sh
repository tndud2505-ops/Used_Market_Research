...
if [[ ! "$IMPORT_URL" =~ ^https:// ]]; then
  printf 'D1 import URL must use HTTPS\n' >&2
  exit 2
fi
...
temp_file="$(mktemp)"
awk -F= -v import_url="$IMPORT_URL" -v import_token="$import_token" '
  BEGIN { url_found = 0; token_found = 0 }
  { sub(/\r$/, "") }
...
rm -f -- "$temp_file"
printf '{"status":"success","d1_url_configured":true,"d1_token_configured":true}\n'
