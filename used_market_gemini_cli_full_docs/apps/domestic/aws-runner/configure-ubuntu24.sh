...
if [[ -n "$import_url" ]]; then
  ...
  [[ -n "$import_token" ]] || fail 'D1_IMPORT_URL을 설정할 때 import token도 필요합니다.'
else
  import_token=''
fi
...
systemctl enable --now used-market-runner.service
systemctl enable --now used-market-tunnel.service
...
log '환경변수와 Tunnel 토큰을 저장하고 두 서비스를 시작했습니다.'
printf '로컬 확인: bash %s/aws-runner/health-check.sh\n' "$APP_ROOT"
