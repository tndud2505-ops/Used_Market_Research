...
- 90개 선읽기 뒤 1~3페이지와 cursor 한 번으로 여는 4페이지를 버튼으로 표시하되 먼 페이지는 비활성 총량으로 둔다.
...
이 하네스 통과는 원 사이트 응답 품질까지 증명하지 않는다. 배포 전에는 `npm run search:matrix:live`와 브라우저에서 `전체 → 사이트 탭 → 3페이지 → 4페이지 → 낮은 가격 → 높은 가격 → 가격 범위 → 전체 → 다른 사이트` 흐름을 별도로 확인한다.
